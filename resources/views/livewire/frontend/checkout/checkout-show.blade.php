...
{{--                                                <button type="button"  wire:loading.attr="disabled" class="btn btn-warning">Pay Now (Online Payment)</button>--}}
            ...
            <div class="card card-body shadow text-center p-md-5">
                ...
                <a href="{{url('collections')}}" class="btn btn-warning">Buy Now</a>
            </div>
            @endif
        </div>
    </div>
...
@push('scripts')
    <script src="https://www.paypal.com/sdk/js?client-id=AffgHKo_D5MZuOy_drZzLUxd0cOybOBq9g5QeVKG4qdI-Be1vanLjjVXqkijbJ8i8GpfGbjyp-T0vdTX&currency=USD"></script>
            ...
            createOrder: (data, actions) => {
                return actions.order.create({
                    purchase_units: [{
                        amount: {
                            value: '0.1'
                            {{--"{{$this->totalProductAmount}}" // Can also reference a variable or function--}}
                        }
                    }]
                });
            },
